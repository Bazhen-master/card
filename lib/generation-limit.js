...
// Сколько генераций оплачено баллами за те же сутки. Отдельного признака в
...
// Сколько БЕСПЛАТНЫХ генераций посетителю ещё доступно за последние сутки.
...
export async function remainingGenerations(supabase, { session, ip, profile }) {
  const since = new Date(Date.now() - WINDOW_MS).toISOString();

  const [byVisitor, byIp, paid] = await Promise.all([
    profile
