...
// хвостом к описанию посетителя. Общий хвост держит колоду в одной манере:
// без надписей и рамок, один говорящий образ на карту.
const BASE_PROMPT =
  "metaphorical association card illustration, single evocative symbolic scene, " +
  "soft depth, no text, no lettering, no watermark, no frame, no border";
...
// Flux работает с размерами, кратными 32.
async function generateImage({ prompt, style, width = 768, height = 1024 }) {
  const preset = STYLES.find((item) => item.id === style);
  const query = [prompt, preset?.prompt, BASE_PROMPT].filter(Boolean).join(", ");

  const started = await api(`/networks/${network}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ model, prompt: query, width, height, num_images: 1 }),
  });
