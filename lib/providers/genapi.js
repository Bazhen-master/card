...
// У сетевых осечек собственное сообщение fetch пустое — «fetch failed», — а всё
// полезное лежит в cause: код вида UND_ERR_SOCKET (связь оборвалась) или
// TimeoutError (вышло время). Его и достаём.
function reason(error) {
  return error?.cause?.code || error?.name || error?.message || "причина неизвестна";
}

// fetch отдаёт управление, как только пришли заголовки: тело докачивается
// после. Поэтому обрыв связи на середине картинки вылетает не из fetch, а из
// чтения тела, и ловить его надо отдельно — иначе наружу уходит сырое
// «TypeError: fetch failed» простынёй стека undici, по которой не понять даже,
// что качали. Посетитель в любом случае увидит вежливый отказ, а вот причина
// нужна словами: она уходит в лог хостинга.
async function download(url) {
  ...
  try {
    ...
    throw new Error(
      `Картинка сгенерирована, но связь для скачивания не открылась: ${reason(error)}`
    );
  }
  ...
  try {
    ...
    throw new Error(`Картинка сгенерирована, но скачалась не целиком: ${reason(error)}`);
  }
}
